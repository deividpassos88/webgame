...
const COMMON_MATERIAL_IDS = COMMON_CRAFT_MATERIAL_IDS;

function profileWithCommonMaterials() {
  const profile = createDefaultPlayerProfile();
  profile.backpack = COMMON_MATERIAL_IDS.map((itemId) => ({ itemId, quantity: 10 }));
  return profile;
}
...
describe('BlacksmithWorkshop', () => {
  ...
  it('defines five common recipes with five ten-unit materials and shared materials across the set', () => {
  ...
  it('crafts a common forged helmet into the backpack without auto-equipping it', () => {
    const profile = profileWithCommonMaterials();
    profile.blacksmith.availableUntil = NOW + 1;

    const result = craftBlacksmithRecipe(profile, 'common-forged-helmet', NOW);
...
function recipeIngredientIds(itemId: string): boolean {
  return BLACKSMITH_RECIPES[0].ingredients.some((ingredient) => ingredient.itemId === itemId);
}
