...
describe('kill rewards', () => {
  it.each([
    ['sword', 'regular', 100, { healAmount: 3, damageBonus: 0.04, maxHpBonus: 0 }],
    ['sword', 'mini-boss', 100, { healAmount: 6, damageBonus: 0.1, maxHpBonus: 15 }],
    ['axe', 'regular', 100, { healAmount: 3.1, damageBonus: 0.04, maxHpBonus: 0 }],
    ['axe', 'mini-boss', 100, { healAmount: 6.2, damageBonus: 0.1, maxHpBonus: 15 }],
  ...
  it('keeps accumulated fractional damage normalized to two decimal places', () => {
    const api = killRewards as typeof killRewards & {
      addDamageBonus?: (current: number, bonus: number) => number;
    };

    expect(api.addDamageBonus?.(8.3, 0.04)).toBe(8.34);
  });
